...
CREATE POLICY "المنتجات متاحة للتعديل للمسؤولين فقط" ON products FOR INSERT WITH CHECK (auth.role() = 'authenticated');
...
CREATE POLICY "الفئات متاحة للتعديل للمسؤولين فقط" ON categories FOR INSERT WITH CHECK (auth.role() = 'authenticated');
